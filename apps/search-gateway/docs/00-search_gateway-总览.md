...
- 项目形态：仓内子项目
- 技术栈：Python
- 运行形态：**常驻 HTTP 网关**（`:3100` API 转发网关 / `:3000` 搜索网关），OpenAI 兼容 `/v1` Base URL 访问，非内嵌库（定案见 [[01-search_gateway-架构与运行.md|01 架构]])
...
未核实的调用方、暴露范围等细节见 `01-search_gateway-架构与运行.md` 的「运行形态定案」小节（进程入口已确认）。
