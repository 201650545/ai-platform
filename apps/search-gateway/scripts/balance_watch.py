...
- zscc：api.zscc.in/dashboard → 解析「剩余额度」文本（余额已耗尽 → 负数记 0）
...
ZSCC_PAGE = "https://api.zscc.in/dashboard"
...
def get_zscc_balance():
    ...
    time.sleep(8)
    val = ocli_eval("() => { const t = document.body.innerText; const i = t.indexOf('剩余额度'); const seg = i>=0 ? t.slice(i, i+120) : ''; const m = seg.match(/[¥￥]-?[\\d,.]+/); return {exhausted: /已耗尽/.test(seg), raw: m ? m[0] : null}; }")
    if isinstance(val, dict):
        if val.get("exhausted"):
            return 0.0
        raw = val.get("raw")
        if raw:
            try:
...
def main():
    ...
    breaches = []
    for cid, remaining in balances.items():
        ch = chs.get(cid)
