...
| 文件 | 说明 |
|---|---|
| `index.json` | 入口：元数据 + 表计数 + 模糊化策略 |
| `capabilities.json` | 资源能力规格表（白名单 18 字段） |
| `instances.json` | 资源实例表（白名单字段 + 计算字段 `额度状态`） |
...
4. **敏感扫描**：产物写完后逐条 regex 扫描，命中即退出码非 0、中止部署。配置在 `config.json` 的 `scan.patterns`。
