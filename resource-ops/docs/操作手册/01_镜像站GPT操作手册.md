...
### 9.3 每窗口 ≤24 轮对话（强制 · **2026-09-18 用户修订：12 → 24**）

- **同一个聊天窗口最多 24 轮对话**（累计计数）。
- 达到 24 轮 → **先再发一轮，要求 GPT 总结上下文**（**不要直接新开**）。
  - 该轮目的：产出一份可迁移的上下文摘要（任务目标 / 已完成的决策 / 当前卡点 / 待办 / 关键约定）。
- 若此时任务仍未完成、需要继续对话 → **带着这份上下文总结新开聊天记录**，补充足够上下文后继续。
- 新开对话后重新选模型（Thinking•Extended）+ 必要时重新注入问题背景。

> 📌 **为什么改 24**：原 12 轮对"长程审查型会话"（本窗口实际已 44 轮，且 GPT 全程精准引用早期约定）过紧；
> 一刀切新开会**丢失已完成决策的连贯上下文**，代价高于上下文膨胀风险。
>
> 🛡 **但 24 轮不是免死金牌**：仍需在接近上限时主动观察 GPT 是否出现**遗忘 / 混淆早期约定 / 答非所问**。
> 一旦出现，不必等满 24 轮 —— **立即走「要总结 → 带总结新开」**（见 §9.5 与 SOP 实测补充 8 的误判教训）。
...
- 24 轮上限：防止单窗口上下文膨胀导致质量下降/成本失控（原为 12 轮，2026-09-18 用户修订为 24，与 §9.3 一致）。
