...
## 9. 会话复用与轮次纪律（用户拍板 2026-08-15，强制）
...
- **每个问题与 GPT 的对话轮次不得超过 3 轮**（同一窗口同一上下文内计数）。
- 3 轮 = 你发问题 + 最多 2 次追问/澄清（含 GPT 反问后的你来我往）。
- GPT 反问时**要继续交流**（回应它的反问、澄清意图），但**必须在 3 轮内把问题解决**，然后回来执行方案。
- 3 轮内没解决 → 停止追问，回来自己判断/或另起新问题（不无限纠缠）。

### 9.3 每窗口 ≤42 轮对话（强制）

- **同一个聊天窗口最多 42 轮对话**（累计计数）。
- 达到 42 轮 → **必须新开对话**（左侧 New chat 或开新会话）。
